...
class SOLD2Trainer(OnlineTrainer):
    def __init__(self, savi: SAVi, env: ImageEnv,
        ...
        regression_infos = {"num_slots": savi.corrector.num_slots, "slot_dim": savi.corrector.slot_dim}
        self.savi = savi
    ...
    def configure_optimizers(self) -> OptimizerLRScheduler:
        model_parameters = list(self.dynamics_predictor.parameters()) + list(self.reward_predictor.parameters())
    ...
    def training_step(self, batch, batch_index: int) -> STEP_OUTPUT:
        model_optimizer, actor_optimizer, critic_optimizer = self.optimizers()
        ...
        outputs |= self.compute_model_loss(images, slots, actions, rewards, is_firsts)
        self.manual_backward(outputs["model_loss"])
        self.clip_gradients(model_optimizer, gradient_clip_val=0.05, gradient_clip_algorithm="norm")
    ...
    def compute_model_loss(self, images: torch.Tensor, slots: torch.Tensor, actions: torch.Tensor, rewards: torch.Tensor, is_firsts: torch.Tensor) -> Dict[str, Any]:
        batch_size, sequence_length, num_slots, slot_dim = slots.shape
        context_slots = slots[:, :self.num_context].detach()

        predictor_input = self.dynamics_predictor._update_buffer_size(context_slots.clone())

        pred_slots, pred_tokens = [], []
        for t in range(predictor_input.shape[1], predictor_input.shape[1] + self.num_predictions):
            input_actions = self.dynamics_predictor._update_buffer_size(actions.clone()[:, :t])
            # print("input_actions.shape:", input_actions.shape)
            token_output, output = self.dynamics_predictor.predictor(predictor_input, input_actions)  # get predicted slots from step
            ...
            predictor_input = self.dynamics_predictor._update_buffer_size(predictor_input)
            pred_slots.append(output)
            pred_tokens.append(token_output)

        future_slots = torch.stack(pred_slots, dim=1)
        future_tokens = torch.stack(pred_tokens, dim=1)  # Predicted hidden states

        print("future_slots.shape:", future_slots.shape)
        print("future_tokens.shape:", future_tokens.shape)
        ...
        predicted_rewards = TwoHotEncodingDistribution(self.reward_predictor(future_tokens, future_slots.detach().clone()), dims=1)
        log_probs = predicted_rewards.log_prob(rewards.detach().unsqueeze(2))
        masked_log_probs = log_probs[~is_firsts]
        reward_loss = -masked_log_probs.mean()

        input()

        return {"slot_loss": slot_loss, "image_loss": image_loss, "reward_loss": reward_loss, "rewards": rewards, "predicted_rewards": predicted_rewards.mean.squeeze(2), "dynamics_loss": slot_loss + image_loss + reward_loss,
...
@hydra.main(config_path="../configs", config_name="sold2")
