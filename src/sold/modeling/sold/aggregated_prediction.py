...
class AggregatedPredictor(nn.Module):
    def __init__(self, num_slots: int, slot_dim: int, token_dim: int, num_heads: int,
        ...
        """Used to predict single quantities like rewards or actions from the set of hidden states and predicted slots."""
        super().__init__()
        num_mlp_layers = 3
    ...
    def forward(self, hidden_states: torch.Tensor, slots: torch.Tensor, start: int = 0) -> torch.Tensor:
...
class GaussianPredictor(AggregatedPredictor):
    def __init__(self, num_slots: int, slot_dim: int, token_dim: int, num_heads: int,
                 num_layers: int, hidden_dim: int, output_dim: int, num_register_tokens: int = 0) -> None:
        super().__init__(num_slots, slot_dim, token_dim, num_heads, num_layers, hidden_dim,
                         output_dim=2*output_dim, num_register_tokens=num_register_tokens)
        self.max_std, self.min_std, self.init_std = 1.0, 0.1, 2.0

    def forward(self, hidden_states: torch.Tensor, slots: torch.Tensor, start: int = 0) -> D.Distribution:
...
class TwoHotPredictor(AggregatedPredictor):
    def __init__(self, num_slots: int, slot_dim: int, token_dim: int, num_heads: int,
